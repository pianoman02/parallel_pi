...
void readvariable(double complex *x, char* st, long n, long decimalsPerRadix){
    //////////////////
    // WARNING: THIS FUNCTION IS OUTDATED AND SHOULDN'T BE USED
    //////////////////
...
void one_over_square_root(double complex *a, double complex *x, long *carry,long radix, long n,long M,double complex *w, long*rho_np, long*rho_p){
...
void square_root(double complex *a, double complex *x, long *carry,long radix, long n,long M,double complex *w, long*rho_np, long*rho_p){
    one_over_square_root(a,x,carry,radix,n,M,w,rho_np,rho_p);
...
void one_over(double complex *a, double complex *x, long *carry,long radix, long n,long M, double complex *w, long*rho_np, long*rho_p){
...
void calculatepi(){
    bsp_begin(P);
    long n;// NOTE: n should be a power of two, as well as p
    ...
    long decimalsPerRadix;
    long M;
    long s = bsp_pid();
    long p = bsp_nprocs();
    bsp_push_reg(&n,sizeof(long));
    bsp_push_reg(&decimalsPerRadix,sizeof(long));
    ...
    if (s==0){
        ...
        scanf("%ld",&decimalsPerRadix);
        printf("How many iterations (M)?\n");
        scanf("%ld",&M);
        for (long q=1; q<p; q++){
            bsp_put(q,&n,&n,0,sizeof(long));
            bsp_put(q,&decimalsPerRadix,&decimalsPerRadix,0,sizeof(long));
    ...
    // Initialising values
    setToInt(aprev,n,2);
    square_root(aprev,a,carry,radix,n,M,w,rho_np,rho_p);
    setToInt(power2,n,1);
    setToInt(b,n,1);
    setToInt(d,n,1);
    for (long i=0; i<M; i++){
        ...
        square_root(aprev,b,carry,radix,n,M,w,rho_np,rho_p);
    ...
    one_over(d,aprev,carry,radix,n,M,w,rho_np,rho_p);
